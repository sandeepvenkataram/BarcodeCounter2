...
def parse_sample_file(sample_file: str, template_array: list[TemplateSequence]) -> list[SampleFileMetadata]:
    ...
    if sample_df.shape[1] < 3 or sample_df.shape[1] > 4:
        raise ValueError(f'The sample file {sample_file} doesnt have 3 or 4 columns, as expected')
    ...
    for _, row in sample_df.iterrows():
        sample_array.append(SampleFileMetadata(row['sample'], row['fastq'], list(row[index_cols])))
    return sample_array
...
def create_const_region_fasta(template_array: list[TemplateSequence], args: dict) -> dict:
    ...
    blastCall = [args['blast_path'] + 'makeblastdb', '-in', args['output_dir'] + ALL_CONST_REGIONS_FILE_NAME, '-dbtype', 'nucl']
