...
from lib.constants import (
    ALL_CONST_REGIONS_FILE_NAME,
    CONSTANT_REGION_BLAST_PARAMS,
    FILE_BUFFER_SIZE,
    MAX_N_IN_READS,
)
            ...
            for bc_record in identified_bc_seq_records:
                ...
                if not final_bc:
                    ...
                    if args['bc_Ngap_length'] > 0:
                        NGap = SeqRecord(Seq('N' * args['bc_Ngap_length']), id='NGap')
                        NGap.letter_annotations['phred_quality'] = [40] * args['bc_Ngap_length']
...
def extract_regions_from_fastq(read_seq_record_list: list[list[SeqIO.SeqRecord]], args: dict, template_array: list[TemplateSequence], template_seq_length_dict: dict):
    ...
    for read_id, read_df in blast_result_df.groupby('read_id'):
        ...
        return_val = [identified_bc_seq_records, identified_umi_sequences, identified_index_bcs]
